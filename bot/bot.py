...
async def api(method: str, path: str, **kwargs):
    ...
    async with httpx.AsyncClient(timeout=15) as http:
        r = await http.request(method, API_URL.rstrip("/") + path, headers=headers, **kwargs)
        r.raise_for_status()
        return r.json()

@tree.command(name="genkey", description="Generate a license key")
@app_commands.describe(key_type="1d = 24 hours, lifetime = no expiry")
...
async def genkey(interaction: discord.Interaction, key_type: app_commands.Choice[str]):
    try:
        ...
        await interaction.response.send_message(f"🔐 `{data['key']}`", ephemeral=True)
    except Exception:
        await interaction.response.send_message("❌ Server error.", ephemeral=True)

@tree.command(name="revoke", description="Revoke a key")
...
@tree.command(name="keys", description="Show key metadata")
async def keys(interaction: discord.Interaction):
    try:
        data = await api("GET", "/admin/list")
        lines = []
        for item in data["licenses"][:25]:
            status = "revoked" if item["revoked_at"] else ("active" if item["activated_at"] else "unused")
            lines.append(f"• {item['key_type']} | {status} | bound: {'yes' if item['device_id'] else 'no'}")
        await interaction.response.send_message("\n".join(lines) or "No keys yet.", ephemeral=True)
    except Exception:
        await interaction.response.send_message("❌ Server error.", ephemeral=True)
...
if not TOKEN or not API_SECRET:
    raise SystemExit("DISCORD_TOKEN and LICENSE_API_SECRET are required")
